...
describe('stream', function()
  ...
  describe('reversed', function()
    it('empty', function()
      assert.are.same({}, stream.reversed({}))
    end)
    it('normal', function()
      assert.are.same({ 3, 1, 2 }, stream.reversed({ 2, 1, 3 }))
      assert.are.same({ 'b', 'a', 'c' }, stream.reversed({ 'c', 'a', 'b' }))
      assert.are.same({ { 'a', 1 }, { 'b', 3 }, { 'c', 2 } }, stream.reversed({ { 'c', 2 }, { 'b', 3 }, { 'a', 1 } }))
    end)
  end)
